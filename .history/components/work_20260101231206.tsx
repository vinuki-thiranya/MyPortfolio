...
export default function Work() {
  ...
  useEffect(() => {
    const handleSectionEntry = () => {
      ...
      // Check if we're entering the work section
      if (rect.top <= viewportHeight * 0.7 && rect.bottom >= viewportHeight * 0.3) {
        ...
        // If scrolling up and section is visible from bottom, start with last card
        if (scrollDirection === 'up' && rect.top > 0) {
          setActiveCardIndex(projects.length - 1)
        }
        // If scrolling down and section is visible from top, start with first card  
        else if (scrollDirection === 'down' && rect.top <= 100) {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      // Detect if entering the work section from below (scrolling up from next section)
      if (rect.bottom > window.innerHeight * 0.3 && rect.top < window.innerHeight * 0.7 && e.deltaY < 0) {
        // If we were below this section and now scrolling up into it, start from last card
        if (!enteredFromBottom && rect.top > 0) {
          setEnteredFromBottom(true)
          setActiveCardIndex(projects.length - 1)
        }
      }
      
      // Reset enteredFromBottom when scrolling down through the section
      ...
      // If we're at the last card and scrolling down, allow normal scroll to go to next section
      if (activeCardIndex === projects.length - 1 && e.deltaY > 0) {
        return
      }

      // If we're at the first card and scrolling up, allow normal scroll to go to previous section
      ...
      if (Math.abs(e.deltaY) > 30) {
        ...
        e.preventDefault() // Prevent small scrolls from moving
      }
    }

    window.addEventListener('wheel', handleWheel, { passive: false })
    return () => window.removeEventListener('wheel', handleWheel)
  }, [activeCardIndex, enteredFromBottom])
  ...
  return (
    <div id="work" ref={sectionRef} className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] relative min-h-screen">
      ...
      >
        <div className="max-w-5xl mx-auto px-4 pb-16">
          ...
          <div className="relative h-[320px]">
            {projects.map((project, index) => {
              ...
              // - Active card: stays at 0
              // - Last card: always stays at 0 (fixed under header)
              // - Previous cards (already seen): go up (-100)
              // - Next cards (not yet seen): come from below (100)
              let yPosition = 0
              if (!isActive) {
                if (isLastCard) {
                  yPosition = 0 // Last card always stays fixed
                } else if (isPreviousCard) {
                  yPosition = -100 // Cards that were seen go up
                } else {
                  yPosition = 100 // Cards not yet seen come from below
                }
              }
              ...
              return (
                <motion.div
                  ...
                  animate={{ 
                    opacity: isActive ? 1 : (isLastCard && activeCardIndex === projects.length - 1) ? 1 : 0,
                    y: yPosition,
                    scale: isActive ? 1 : 0.95,
                    zIndex: isActive ? 10 : isLastCard ? 5 : 0
                  }}
                  transition={{ 
                    duration: 0.5, 
        ...
        <motion.div
          className="mt-16 flex justify-between items-center max-w-5xl mx-auto px-4"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ 
            duration: 0.8, 
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 text-[#64ffd9]/60 text-sm font-poppins">
