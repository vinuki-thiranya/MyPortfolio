...
export default function Work() {
  ...
  return (
    <div id="work" className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] min-h-screen relative">
      ...
      {/* Projects Container */}
      <div ref={containerRef} className="relative px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36" style={{ paddingTop: '80vh' }}>
          ...
          <div className="space-y-8">
            {projects.map((project, index) => (
              <div
                ...
                style={{
                  zIndex: index === 0 ? 30 : 35 + index, // First card lower, subsequent cards stack higher
                  top: index === 0 ? '200px' : `200px`, // First card fixed, others stack at same level
                  left: index === 0 ? '50%' : 'auto',
                  transform: index === 0 ? 'translateX(-50%)' : 'none',
                  width: index === 0 ? 'calc(100% - 2rem)' : 'auto',
                  maxWidth: index === 0 ? '800px' : 'auto',
                  marginBottom: index === 0 ? '0' : '60vh' // No margin for fixed first card
                }}
