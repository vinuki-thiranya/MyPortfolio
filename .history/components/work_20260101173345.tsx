...
export default function Work() {
  ...
  // Calculate the total height needed for the stacking effect
  const cardHeight = 320 // Height of each card
  const headerHeight = 260 // Height of the sticky header
  const cardSpacing = 40 // Spacing between cards when stacked (visible portion)

  return (
    <div id="work" ref={sectionRef} className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] relative">
      <style jsx>{`
        .stacking-card {
          position: sticky;
      ...
      {/* Projects Container - Height based on number of cards for scroll space */}
      <div 
        ref={containerRef} 
        className="relative px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36"
        style={{ 
          minHeight: `${projects.length * 60}vh` // Enough scroll space for all cards
        }}
      >
          ...
          <div className="relative">
            {projects.map((project, index) => (
              <div
                ...
                className="stacking-card transition-shadow duration-300"
                style={{
                  zIndex: 10 + index, // Each card stacks above the previous
                  top: `${headerHeight + (index * cardSpacing)}px`, // Each card slightly lower than previous
                  marginBottom: index === projects.length - 1 ? '40vh' : `${cardHeight - cardSpacing}px`, // Last card needs extra space
                }}
