...
export default function Work() {
  ...
  // Handle scroll within the work section to snap between cards
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      // Only handle if the work section is in view
      if (rect.top > window.innerHeight || rect.bottom < 0) return

      // If the work section header is visible and we're at the cards area
      if (rect.top <= 300 && rect.bottom > window.innerHeight) {
        // Prevent default scroll if we're between cards
        if (isScrollingRef.current) {
          e.preventDefault()
          return
        }

        // Require a strong scroll push (deltaY > 40)
        if (Math.abs(e.deltaY) > 40) {
          e.preventDefault()
          isScrollingRef.current = true

          let newIndex = currentCardIndex
          if (e.deltaY > 0 && currentCardIndex < projects.length - 1) {
          ...
          if (newIndex !== currentCardIndex) {
            ...
            const targetCard = projectRefs.current[newIndex]
            if (targetCard) {
  ...
  return (
    <div id="work" className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] relative">
