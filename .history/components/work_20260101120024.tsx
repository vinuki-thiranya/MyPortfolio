  ...
  return (
    <div id="work" className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] min-h-screen relative">
      <style jsx>{`
        ...
        /* Ensure header section stays fixed and creates proper spacing */
        .header-section {
          min-height: 280px;
        }
      `}</style>
      ...
      <div className="header-section sticky top-0 z-50 bg-gradient-to-br from-[#0a192f] to-[#0f172a] pt-12 pb-6 px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36 border-b border-[#1a365d]/30">
        <div className="max-w-[900px] mx-auto">
          <div>
            ...
            <motion.p 
              className="w-full mb-6 text-[#4a5568] dark:text-[#e6f1ff]/70 font-poppins text-base leading-7 max-w-2xl"
      ...
      <div ref={containerRef} className="relative px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36">
          ...
          <div className="space-y-8">
            {projects.map((project, index) => (
              <div
                ...
                data-scroll-section
                className={`project-card transition-all duration-500 ease-out sticky ${
                  visibleProjects.includes(project.id)
                    ? "opacity-100 translate-y-0 scale-100"
                    : "opacity-0 translate-y-32 scale-90"
                } ${index === projects.length - 1 ? 'last-card-fixed' : ''}`}
                style={{
                  zIndex: index === 0 ? 30 : 30 + index,
                  top: index === projects.length - 1 ? '300px' : '280px', // Last card positioned lower to stay under header
                  marginBottom: index === projects.length - 1 ? '60vh' : '30vh' // Adequate margin for last card
                }}
        ...
        <motion.div
          className="mt-16 mb-8 flex justify-end max-w-5xl mx-auto px-4"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ 
            duration: 0.8, 
