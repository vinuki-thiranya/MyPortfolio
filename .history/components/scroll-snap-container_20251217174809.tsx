...
export function useScrollSnap(options: UseScrollSnapOptions = {}) {
  ...
  const canNavigateToNextSection = useCallback((sectionIndex: number, scrollDirection: 'up' | 'down') => {
    ...
    if (scrollDirection === 'down') {
      // Check if we've scrolled past the section bottom OR reached the very end of the document
      const pastSectionBottom = viewportBottom >= sectionBottom + 10
      const atDocumentEnd = scrollTop + windowHeight >= documentHeight - 10
      
      // Only navigate if we're past the section or at document end
      return pastSectionBottom || atDocumentEnd
    } else {
      // Check if we've scrolled above the section top OR reached the very beginning of the document
      const aboveSectionTop = viewportTop <= sectionTop - 10
      const atDocumentStart = scrollTop <= 10
      
      // Only navigate if we're above the section or at document start
      return aboveSectionTop || atDocumentStart
    }
  }, [])
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    scrollTimeoutRef.current = setTimeout(() => {
      if (Math.abs(wheelDeltaRef.current) > threshold) {
        ...
        // Only snap if we can navigate to the next section (past current section boundaries)
        if (canNavigateToNextSection(currentSectionIndex, scrollDirection)) {
          e.preventDefault()
          ...
          // Reset wheel delta if we're not at the section end
          wheelDeltaRef.current = 0
        }
      } else {
        wheelDeltaRef.current = 0
  ...
  }, [threshold, duration, getCurrentSection, scrollToSection, canNavigateToNextSection])
