...
export default function Work() {
  ...
  const isEnteringFromTopRef = useRef(true)

  // Check if section is in viewport
  const isInView = useInView(sectionRef, {
    margin: "-50% 0% -50% 0%", // Trigger when 50% of section is in view
  })

  // Set initial active card based on entry direction
  useEffect(() => {
    if (isInView && !isInViewRef.current) {
      // Just entered the viewport
      const sectionRect = sectionRef.current?.getBoundingClientRect()
      const viewportHeight = window.innerHeight
      
      if (sectionRect) {
        // If section top is above viewport middle, we're entering from top
        const isEnteringFromTop = sectionRect.top < viewportHeight / 2
        isEnteringFromTopRef.current = isEnteringFromTop
        
        if (isEnteringFromTop) {
          setActiveCardIndex(0) // Start from first card
        } else {
          setActiveCardIndex(projects.length - 1) // Start from last card
        }
      }
    }
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      if (!isInView || isScrollingRef.current) return

      const section = sectionRef.current
      if (!section) return

      const rect = section.getBoundingClientRect()
      // Only handle when work section is in the middle of viewport
      ...
      // Check boundaries before allowing navigation
      ...
      // If at boundaries, let the main scroll snap handle it
      ...
      if (e.deltaY > 0) {
        ...
        setActiveCardIndex(prev => Math.min(prev + 1, projects.length - 1))
      } else {
  ...
  }, [activeCardIndex, isInView])

  // Handle card animation based on active index
  ...
  return (
    <div 
      ...
      ref={sectionRef} 
      data-scroll-section
      className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] relative min-h-screen snap-start"
    >
      
      {/* Fixed Header Section */}
      <div className="sticky top-0 z-40 bg-gradient-to-br from-[#0a192f]/95 to-[#0f172a]/95 backdrop-blur-sm pt-12 pb-4 px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36">
      ...
      <div 
        ref={containerRef} 
        className="relative px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36 pt-4 pb-16 min-h-[500px]"
      >
        <div className="max-w-5xl mx-auto px-4 pb-16 relative">
          {/* Single card display area */}
          <div className="relative h-[320px]">
            {projects.map((project, index) => {
              const animationState = getCardAnimationState(index)
              ...
              return (
                <motion.div
                  ...
                  style={{ 
                    pointerEvents: animationState.opacity === 1 ? 'auto' : 'none',
                    visibility: animationState.visible ? 'visible' : 'hidden',
        ...
        <motion.div
          className="mt-4 flex justify-end max-w-5xl mx-auto px-4"
