...
const projects: Project[] = [
  {
    id: 1,
    title: "FormLang++ – DSL for HTML Form Generation",
    description: "Developed a domain-specific language (DSL) called FormLang++ to simplify and automate creating dynamic, validated HTML forms using a declarative, human-readable syntax. Built using C, Flex (Lex), and Bison (Yacc).",
  ...
  },
  {
    ...
    description: "Developed a fault-tolerant distributed logging system ensuring high availability and reliability during node failures. Specialized in Time Synchronization & Fault Tolerance using NTP and heartbeat algorithms.",
  ...
  {
    id: 4,
    title: "Dogwood Flora – Online Floral Shop System",
  ...
  },
  {
    ...
    description: "Developed a smart safety helmet for miners with sensors for gas, heart rate, temperature, and pressure, plus a real-time monitoring web dashboard.",
    image:  "/images/Intellihelmet2.png",
  ...
  {
    id: 7,
    title: "AutoParts Hub – SME Mobile App (UI/UX)",
    description: "Designed a comprehensive mobile application UI/UX solution to empower a Sri Lankan automobile spare parts SME by improving digital presence, customer engagement, and operational efficiency.",
    image: "/images/autoparts-high-fidelity.png",
    technologies: ["Figma", "UI/UX Design", "HCI", "User Research", "Prototyping"],
...
export default function Work() {
  ...
  const [isLastCardFixed, setIsLastCardFixed] = useState(false)
    ...
    const handleSectionEntry = () => {
      ...
      if (isInView && !wasInViewRef.current) {
        ...
        if (scrollDirection === 'up') {
          ...
          setIsLastCardFixed(true)
        } else {
          setActiveCardIndex(0)
          setIsLastCardFixed(false)
        }
      }
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      // If at last card and scrolling down, allow normal scroll to next section
      if (isAtLastCardAndScrollingDown) {
        // Let the browser handle the scroll naturally
        setIsLastCardFixed(true)
        return
      }

      // If at first card and scrolling up, allow normal scroll to previous section
      if (isAtFirstCardAndScrollingUp) {
        return
      }
      ...
      if (Math.abs(e.deltaY) > 30) {
        ...
        if (e.deltaY > 0) {
          if (activeCardIndex < projects.length - 1) {
            ...
            // If moving to last card, set it as fixed
            if (activeCardIndex === projects.length - 2) {
              setIsLastCardFixed(true)
            } else {
              setIsLastCardFixed(false)
            }
          ...
          if (activeCardIndex > 0) {
            setActiveCardIndex(prev => prev - 1)
            // If moving away from last card, unset fixed
            if (activeCardIndex === projects.length - 1) {
              setIsLastCardFixed(false)
            }
          }
  ...
  return (
    <div id="work" ref={sectionRef} className="bg-gradient-to-br from-[#0a192f] to-[#0f172a] relative min-h-screen">
      ...
      {/* Projects Container */}
      <div ref={containerRef} className="relative px-6 sm:px-10 md:px-14 lg:px-20 xl:px-28 2xl:px-36 min-h-[500px]">
        <div className="max-w-5xl mx-auto px-4 pb-16 relative">
          <div className="relative h-[320px]">
            {projects.map((project, index) => {
              ...
              // Special handling for last card when it's active and fixed
              if (isLastCard && isLastCardFixed && isActive) {
                return (
                  <div
                    key={project.id}
