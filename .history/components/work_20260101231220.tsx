...
export default function Work() {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      // If we're at the last card and scrolling down, allow normal section navigation
      if (activeCardIndex === projects.length - 1 && e.deltaY > 0) {
        return // Let section navigation handle it
      }

      // If we're at the first card and scrolling up, allow normal section navigation
      ...
      if (Math.abs(e.deltaY) > 30) {
        ...
        if (e.deltaY > 0) {
          // Scrolling down - next card (forward through array: 0 -> 1 -> 2 -> ... -> last)
          if (activeCardIndex < projects.length - 1) {
            setActiveCardIndex(prev => prev + 1)
          }
        } else {
          // Scrolling up - previous card (backward through array: last -> ... -> 2 -> 1 -> 0)  
          if (activeCardIndex > 0) {
            setActiveCardIndex(prev => prev - 1)
          }
        }
      } else {
        e.preventDefault() // Prevent small scrolls
      }
    }
