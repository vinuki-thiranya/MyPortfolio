...
const projects: Project[] = [
  {
    ...
    description:
      "A comprehensive domain-specific language (DSL) designed to revolutionize HTML form creation through high-level abstraction. FORMLANG++ transforms structured .form source files into clean, standards-compliant HTML5 with built-in validation and robust error handling. Built from scratch using C, Flex, and Bison, this project demonstrates complete compiler pipeline implementation including lexical analysis, parsing, AST construction, semantic validation, and code generation. The DSL addresses common web development pain points like repetitive markup, inconsistent validation logic, and poor maintainability by providing an expressive, readable syntax that focuses on form logic rather than implementation details. Features comprehensive error detection, semantic validation rules, and generates optimized HTML5 output with embedded validation attributes.",
    image:
  ...
  {
    id: 7,
    title: "AutoParts Hub – SME Mobile Application (UI/UX Design)",
    description:
      "Designed a comprehensive mobile application UI/UX solution to empower a Sri Lankan automobile spare parts SME by improving digital presence, customer engagement, and operational efficiency. Applied HCI principles focusing on usability, accessibility, and real-world SME needs.",
    image:
      "/images/autoparts-high-fidelity.png",
    technologies: ["Figma", "UI/UX Design", "HCI", "User Research", "Prototyping", "Wireframing"],
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-6 md:p-12">
        ...
        >
          ...
          <nav className="flex items-center space-x-8">
            ...
            <Link 
              href="/#technologies" 
              className="font-poppins text-[#2d3748] dark:text-white/70 text-sm font-medium hover:text-[#667eea] dark:hover:text-[#64ffd9] transition-colors duration-200 uppercase tracking-widest"
            >
              SKILLS
            </Link>
            <Link 
              href="/about" 
              className="font-poppins text-[#667eea] dark:text-[#64ffd9] text-sm font-medium hover:text-[#4c51bf] dark:hover:text-[#4fd1c7] transition-colors duration-200 uppercase tracking-widest"
            >
              ABOUT
            </Link>
          </nav>
                ...
                <div className="relative z-10 p-5 flex-1 flex flex-col justify-end">
                  ...
                  <div className="flex gap-2.5">
                    ...
                    <a
                      ...
                      className="flex-1 inline-flex items-center justify-center px-3 py-2.5 bg-white/30 text-white hover:bg-white hover:text-[#0a192f] border border-white/50 text-xs font-semibold rounded-lg transition-all duration-300 hover:scale-105 hover:shadow-lg transform active:scale-95 backdrop-blur-md"
                      onClick={(e) => e.stopPropagation()}
                    >
