...
interface CaseStudyDetail {
  ...
  timeline: { phase: string; outcome: string; duration: string }[]
  impact: string
  accent: string
}
...
const caseStudies: Record<string, CaseStudyDetail> = {
  "formlang-plus-plus": {
    tagline: "COMPILER-GRADE FORM GENERATION",
    summary:
      "FormLang++ transforms compiler theory into a domain-specific language that produces validated HTML forms from a human-readable syntax, making form generation repeatable and auditable.",
    sections: [
      {
        title: "Grammar-first design",
        detail:
          "Built a Flex/Bison grammar that understands layout directives, input types, and validation flags, ensuring the parser produces complete ASTs in a single pass."
      },
      {
        title: "Declarative UX contracts",
        detail:
          "Each form block is described in FormLang++ using human-friendly declarations, so designers can describe intent while the compiler generates semantic markup."
      },
      {
        title: "Validation as code",
        detail:
          "Semantic analyzers enforce required fields, numeric ranges, and regex rules while returning actionable diagnostics whenever rules break."
      }
    ],
    metrics: [
      { label: "Grammar tokens", value: "48 tokens across 6 production clusters" },
      { label: "Forms generated", value: "120+ dynamic layouts with validations" },
      { label: "Validation rules", value: "15 built-in semantic checks" },
      { label: "Compile-time speed", value: "< 350ms per form on local VM" }
    ],
    timeline: [
      {
        phase: "Parser & AST",
        outcome: "Lexical analyzer exposes tokens to Bison, producing AST nodes for controls and layout directives.",
        duration: "3 weeks"
      },
      {
        phase: "Code generation",
        outcome: "Template engines emit accessible HTML/CSS plus inline validation shims and error placeholders.",
        duration: "2 weeks"
      },
      {
        phase: "Validation QA",
        outcome: "Regression harness runs 40+ form cases to confirm semantic, syntactic, and UX expectations.",
        duration: "2 weeks"
      }
    ],
    impact:
      "The result was a repeatable, compiler-checked workflow for HTML forms that saved hours of manual markup while keeping validation logic centralized.",
    accent: "from-[#0a192f] via-[#112240] to-[#1f2a3d]"
  },
  "auto-service-management-system": {
    tagline: "SPARE PARTS ECOSYSTEM",
    summary:
      "The Auto Service Spare Parts platform unifies authentication, role-aware workflows, and a responsive catalogue so mechanics and managers can operate with confidence.",
    sections: [
      {
        title: "Security-first auth",
        detail:
          "JWT-based tokens guard every endpoint, supported by refresh flows, email verification, and centralized exception handling for auditability."
      },
      {
        title: "Operational catalog",
        detail:
          "Responsive dashboards render 3,400+ spare parts with filters, specs, and real-time stock indicators so staff can quickly locate components."
      },
      {
        title: "Admin precision",
        detail:
          "Role-based authorization surfaces unique CRUD panels for admins, service managers, and technicians with contextual action buttons."
      }
    ],
    metrics: [
      { label: "Secure sessions", value: "JWT lifecycle w/ refresh + 10m token TTL" },
      { label: "Catalog footprint", value: "3,400 spare part records with metadata" },
      { label: "Dashboards", value: "5 role-aware dashboards" },
      { label: "Latency", value: "< 200ms average API response" }
    ],
    timeline: [
      {
        phase: "Security blueprint",
        outcome: "Implemented Spring Security, JWT handling, password reset tokens, and audit logging for every auth action.",
        duration: "4 weeks"
      },
      {
        phase: "UX + catalog",
        outcome: "Crafted responsive React/Tailwind views that let staff filter parts, visualize stock, and navigate user roles.",
        duration: "3 weeks"
      },
      {
        phase: "Monitoring & launch",
        outcome: "Rolled out enhanced exception handlers, API health checks, and Jira-based QA tickets before the demo release.",
        duration: "2 weeks"
      }
    ],
    impact:
      "The project delivered a production-ready suite that automated spare part visibility while locking down access for sensitive inventory controls.",
    accent: "from-[#0a192f] via-[#0b1a2a] to-[#171a2c]"
  }
}
