...
export function useScrollSnap(options: UseScrollSnapOptions = {}) {
  ...
  const isAtSectionBottom = useCallback((section: HTMLElement) => {
    ...
    // Consider at bottom if the viewport bottom is within 50px of section bottom
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    if (Math.abs(e.deltaY) > 20) {
      ...
      // Special handling for sections that need internal scrolling (experience, work, and contact)
      if (currentSection.id === 'experience' || currentSection.id === 'work' || currentSection.id === 'contact') {
