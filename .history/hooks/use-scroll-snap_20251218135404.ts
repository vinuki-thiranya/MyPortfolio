...
export function useScrollSnap(options: UseScrollSnapOptions = {}) {
  ...
  const isInJourneySection = useCallback(() => {
    ...
    // Check if we're within the journey section
    return rect.top <= 0 && rect.bottom > viewportHeight
  }, [])

  const canScrollInJourneySection = useCallback((direction: 'up' | 'down') => {
    const journeySection = document.getElementById('experience')
    if (!journeySection) return false
    ...
    if (direction === 'down') {
      // Can scroll down if we haven't reached the bottom of the journey section
      return rect.bottom > viewportHeight + 100 // 100px buffer
    } else {
      // Can scroll up if we haven't reached the top of the journey section
      return rect.top < -100 // 100px buffer
    }
  }, [])
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    if (isInJourneySection()) {
      ...
      // Allow natural scrolling within the journey section
      if (canScrollInJourneySection(scrollDirection)) {
  ...
  }, [threshold, duration, getCurrentSection, scrollToSection, isInJourneySection, canScrollInJourneySection])
