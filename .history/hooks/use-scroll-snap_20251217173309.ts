...
export function useScrollSnap(options: UseScrollSnapOptions = {}) {
  ...
  const isAtSectionEnd = useCallback((sectionIndex: number, scrollDirection: 'up' | 'down') => {
    ...
    if (scrollDirection === 'down') {
      // Check if we've reached the bottom of the section
      // Allow some small tolerance (50px) for precision
      return viewportBottom >= sectionBottom - 50
    } else {
      // Check if we've reached the top of the section
      // Allow some small tolerance (50px) for precision
      return viewportTop <= sectionTop + 50
    }
  }, [])
