...
export function useScrollSnap(options: UseScrollSnapOptions = {}) {
  ...
  const isAtSectionBottom = useCallback((section: HTMLElement) => {
    ...
    // For work section, be more precise about bottom detection
    // Only consider at bottom when we're actually at the very bottom
    if (section.id === 'work') {
      return viewportBottom >= sectionBottom - 5
    }
    ...
    if (Math.abs(e.deltaY) > 20) {
      ...
      if (currentSection.id === 'experience' || currentSection.id === 'work') {
        ...
        if (e.deltaY > 0) {
          ...
          // For work section, the Work component already handled the transition
          // Just let the natural scroll happen to the next section without snapping
